...
public class ResourceHandlerTest {
	...
	public void init(){
		...
		try {
			...
			jsonObject.put("@prop", "type");
			jsonObject.put("@nv", "WOOD");
	...
	@Test
	public void testAddNewResource() throws JSONException, Exception{
		init();
		...
		for (Iterator<SectorElement> sectorElementIter = game.iteratorOfSectorElement(); sectorElementIter.hasNext();) {
			...
			if(id.equals(sectorElement.getId())) {
				resource = (Resource)sectorElement;
			}					
		}
	...
	public void testChangeLevel() throws JSONException, Exception{
		...
		Assert.assertNotNull(resource);
		Assert.assertTrue("IRON".equals(resource.getType()));
	}
}
